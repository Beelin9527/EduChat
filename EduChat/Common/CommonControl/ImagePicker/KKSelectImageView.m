...
//            DWDLog(@"imageUrl:%@", contentEditingInput.fullSizeImageURL);
//        }];
...
{
//    UICollectionReusableView *reusableview = [[UICollectionReusableView alloc] initWithFrame:(CGRect){0, 0, DWDScreenW, 30}];
//    reusableview.backgroundColor = [UIColor redColor];
//    UILabel *la = [UILabel new];
//    la.textColor = DWDColorBody;
...
//
    if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
        ...
        reusableview.backgroundColor = DWDColorBackgroud;
        UILabel *la = [UILabel new];
        la.text = @"添加图片";
        la.textColor = DWDColorSecondary;
        la.font = [UIFont systemFontOfSize:14];
        la.frame = (CGRect){10, 10, DWDScreenW, 15};
        [reusableview addSubview:la];
    }
