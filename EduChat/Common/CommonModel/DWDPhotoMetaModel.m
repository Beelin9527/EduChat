...
@implementation DWDPhotoMetaModel
...
- (CGSize)thumbSize {
    ...
    if (photoHeight > photoWidth) {
        //高 大于 宽
        scaleWidth = pxToW(320);
        scaleHeight = pxToW(320) / photoWidth * photoHeight;
        if (scaleWidth == 0 || isnan(scaleWidth)) {
            scaleWidth = scaleHeight;
        }
    } else {
        //高 小于等于 宽
        scaleHeight = pxToW(320);
        scaleWidth = pxToW(320) / photoHeight * photoWidth;
...
- (CGSize)chatThumbSize {
    ...
    if (photoHeight > photoWidth) {
        ...
        if (photoHeight / photoWidth > 4.0) {
            scaleWidth = pxToW(80);
            scaleHeight = pxToW(80) / photoWidth * photoHeight;
            if (scaleHeight == 0 || isnan(scaleHeight)) {
                scaleHeight = scaleWidth;
            }
        } else {
        ...
        if (photoWidth / photoHeight > 4.0) {
            scaleHeight = pxToW(80);
            scaleWidth = pxToW(80) / photoHeight * photoWidth;
            if (scaleWidth == 0 || isnan(scaleWidth)) {
                scaleWidth = scaleHeight;
            }
        } else {
...
//    DWDMarkLog(@"长边优先photoKey:%@", imgKey);
    return imgKey;
}
...
//    photoMeta {photokey:, width:, height:};
    ...
    DWDMarkLog(@"短边优先photoKey:%@", imgKey);
    return imgKey;
}
...
- (id)copyWithZone:(NSZone *)zone {
    DWDPhotoMetaModel *model = [[DWDPhotoMetaModel allocWithZone:zone] init];
