//
//  DWDRelayChooseClassController.m
//  EduChat
//
...
@interface DWDRelayChooseClassController () <UIAlertViewDelegate>
@property (nonatomic , strong) NSArray *chooseClasses;
@property (nonatomic , strong) DWDClassModel *selectedModel;

@end

@implementation DWDRelayChooseClassController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"选择班级";
    
    self.chooseClasses = [[DWDClassDataBaseTool sharedClassDataBase] getClassList:[DWDCustInfo shared].custId];
    
    // test
//    NSArray *hehe = @[@"张" , @"林" , @"来" ,@"招" , @"哦" , @"没" , @"中" , @"额" , @"饿" , @"啊" , @"锕" , @"吖" , @"把" , @"不", @"别" , @"过" , @"都"];
//    NSMutableArray *yaya = [NSMutableArray array];
//    for (int i = 0; i < hehe.count; i++) {
//        DWDClassModel *contact = [[DWDClassModel alloc] init];
//        contact.className = hehe[i];
//        [yaya addObject:contact];
//    }
//    self.chooseClasses = yaya;
    // test
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    return self.chooseClasses.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    DWDClassModel *model = self.chooseClasses[indexPath.row];
    [cell.iconView sd_setImageWithURL:[NSURL URLWithString:model.photoKey] placeholderImage:DWDDefault_GradeImage];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    self.selectedModel = self.chooseClasses[indexPath.row];
    NSString *string = [NSString stringWithFormat:@"发送给%@?",_selectedModel.className];
...
- (DWDBaseChatMsg *)handleRelayingMsg{
    ...
    if ([base.msgType isEqualToString:kDWDMsgTypeText]) {
        ...
        DWDTextChatMsg *relayMsg = [[[DWDChatMsgClient alloc] init] createTextMsgWithText:textMsg.content from:[DWDCustInfo shared].custId to:self.selectedModel.classId chatType:DWDChatTypeClass];
        ...
        DWDAudioChatMsg *relayMsg = [[[DWDChatMsgClient alloc] init] creatAudioMsgFrom:[DWDCustInfo shared].custId to:self.selectedModel.classId duration:audioMsg.duration observe:nil mp3FileName:audioMsg.fileName chatType:DWDChatTypeClass];
        ...
        DWDImageChatMsg *relayMsg = [[[DWDChatMsgClient alloc] init] creatImageMsgFrom:[DWDCustInfo shared].custId to:self.selectedModel.classId observe:nil chatType:DWDChatTypeClass];
        ...
        DWDVideoChatMsg *relayMsg = [[[DWDChatMsgClient alloc] init] creatVideoMsgFrom:[DWDCustInfo shared].custId to:self.selectedModel.classId observe:nil mp4FileName:videoMsg.fileName chatType:DWDChatTypeClass];
