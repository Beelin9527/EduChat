...
//    DWDMarkLog(@"expandbutton%d", growupModel.expandButtonOn);
//    DWDMarkLog(@"expandbody%d", growupModel.expandBody);
    ...
    DWDMarkLog(@"addtime:%@", growupModel.author.addtime);
    ...
    DWDGrowUpBodyLabel *testLabel = [[DWDGrowUpBodyLabel alloc] init];
    testLabel.text = growupModel.record.content;
    CGRect bounds = CGRectMake(0, 0, DWDScreenW - pxToW(20) - pxToW(57), 200);
    CGRect labelRect = [testLabel textRectForBounds:bounds limitedToNumberOfLines:3];
    CGSize bodySize = labelRect.size;
    //完整内容
    NSDictionary *attr = @{NSFontAttributeName : DWDFontContent};
    CGSize allBodySize = [growupModel.record.content boundingRectWithSize:CGSizeMake(DWDScreenW - pxToW(20) - pxToW(57), MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:attr context:nil].size;
    
    if (allBodySize.height > bodySize.height) {
        growupModel.expandBody = YES;
    } else {
        growupModel.expandBody = NO;
    }
    
    //内容label
    if (growupModel.expandButtonOn) {
        _bodyLabelF = CGRectMake(CGRectGetMaxX(_midLittleImageF) + pxToW(8), CGRectGetMaxY(_iconViewF) + pxToH(20), allBodySize.width, allBodySize.height);
    } else {
...
// 111111111111
    ...
    int col;
    int row;
    [self.picturesFrames removeAllObjects];
