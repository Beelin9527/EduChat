...
-(void)markSelected:(BOOL)blnSelected
{
    ...
    if([self.date isDateToday]){
        self.dateLabelContainer.backgroundColor = (blnSelected)?[UIColor whiteColor]: [UIColor colorWithHex:0xb2cbff];
...
-(UIColor *)colorByDate
{
    return [self.date isPastDate]?[UIColor whiteColor]:[UIColor colorWithHex:0x97b9ff];
}
