...
//        [hud hide:YES];
        ...
        //检测是否为空数组 是则return
        NSArray *arrData = responseObject[DWDApiDataKey];
        if (arrData.count == 0) {
            return ;
        }
...
- (void)requestAddEntityWithParams:(NSDictionary *)params
                       success:(void (^)(id responseObject))success
                       failure:(void (^)(NSError *error))failure
{
   DWDProgressHUD *hud = [DWDProgressHUD showHUD];
    
    [[HttpClient sharedClient] postApi:@"NoteRestService/addEntity" params:params success:^(NSURLSessionDataTask *task, id responseObject) {
