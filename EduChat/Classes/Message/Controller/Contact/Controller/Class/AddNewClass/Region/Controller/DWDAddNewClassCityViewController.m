...
- (void)rightBarBtnClick{
    ...
    //this is meDetail area --- Bharal this is good boy *********** ation *********
    if (self.type == DWDSelfClassPropertyTypeSelectCityForChangeMeArea) {
        
        DWDTownModel *town = self.townArray[self.indexpath.row];
        
        //request
        [[DWDRequestMeDetail sharedRequestMeDetail] requestServerUpdateUserInfoCustId:[DWDCustInfo shared].custId DictParam:@{@"districtCode":town.regionCode} success:^(id responseObject) {
    ...
    //this is meDetail area --- Bharal this is good boy *********** end *********

    
    [[NSNotificationCenter defaultCenter] postNotificationName:DWDDWDCityNameDidSelectNotification object:nil userInfo:dict];
    
    [self.navigationController popToViewController:_addNewVc animated:YES];
}
...
- (void)sectionClick:(DWDCitySectionButton *)btn{
    ...
    if (_selectSection == btn.tag) {
        ...
        hud.labelText = @"正在获取行政区,请骚候";
        NSDictionary *params = @{@"districtCode" : city.regionCode,
                                 @"subdistrict" : @1,
                                 };
        [[HttpClient sharedClient] getApi:@"DistrictRestService/getEntity" params:params success:^(NSURLSessionDataTask *task, id responseObject) {
            ...
            for (int i = 0; i < arr.count; i ++) {
                DWDTownModel *town = [DWDTownModel yy_modelWithJSON:arr[i]];
                [self.townArray addObject:town];
                [_lastBtn setImage:[UIImage imageNamed:@"ic_up"] forState:UIControlStateNormal];
            }
