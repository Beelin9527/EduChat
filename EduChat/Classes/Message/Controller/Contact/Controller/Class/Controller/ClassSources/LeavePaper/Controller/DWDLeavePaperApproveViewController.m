...
- (void)viewDidLoad {
    ...
    [self.view addSubview:[self setupTableFootView]];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row==1) {
        ...
        if (self.leavePaperType == disagreeType) {
            ...
            approveDisagreeCell.createTime.text = self.authorEntity.addTime;
        ...
        applyTimeCell.createTime.text = self.authorEntity.addTime;
...
- (void)requestApproveDidselectbutton:(UIButton *)sender
{
    ...
    NSString *opinion = sender.tag == 0 ? @"" : self.laevePaperApproveDisagreeCell.disagreeOpinion.text;
     ...
     __weak __typeof(self) weakSelf = self;
    [[DWDRequestServerLeavePaper sharedDWDRequestServerLeavePaper] requestApproveNoteCustId:@4010000005410 classId:@8010000001047 noteId:self.noteEntity.noteId state:state opinion:opinion success:^(id responseObject) {
